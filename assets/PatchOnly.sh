#!/bin/sh
# APatch boot-image patcher, file mode only (POSIX sh).
...
ui_print "****************************"
ui_print " APatch Boot Patcher"
ui_print " (file mode - no auto-flash)"
ui_print "****************************"

# Locate stock boot image on sdcard
SRC=""
for f in /sdcard/APatch-stock-boot.img /data/media/0/APatch-stock-boot.img \
...
if [ -z "$SRC" ]; then
  ...
  ui_print "  APatch-stock-boot.img"
...
OUT="$OUTDIR/APatch-patched-$KIND.img"
...
mkdir -p "$OUTDIR/Download/APatch/BootBackups" 2>/dev/null
run_cp -f "$SRC" "$OUTDIR/Download/APatch/BootBackups/" 2>/dev/null
if [ -f "$WORK/APatch.apk" ]; then
  run_cp -f "$WORK/APatch.apk" "$OUTDIR/APatch-Manager.apk" 2>/dev/null
  run_cp -f "$WORK/APatch.apk" "$OUTDIR/Download/APatch/APatch-Manager.apk" 2>/dev/null
fi
...
ui_print " Then reboot and install APatch-Manager.apk"
