#!/bin/sh
# APatch uninstaller / restore (POSIX sh, recovery compatible).
...
BACKUP=""
for d in /data/APatch-Backup /sdcard/APatch-Backup /data/media/0/APatch-Backup \
          /external_sd/APatch-Backup /sdcard/Download/APatch/BootBackups \
          /data/media/0/Download/APatch/BootBackups; do
...
if [ -z "$BACKUP" ]; then
  for d in /data/APatch-Backup /sdcard/APatch-Backup /data/media/0/APatch-Backup \
            /external_sd/APatch-Backup /data /sdcard /data/media/0 /external_sd; do
...
if [ -n "$BACKUP" ]; then
  ...
  for _d in /data/adb/apd /data/adb/ap; do
...
ui_print "****************************"
ui_print " APatch removed. Reboot."
ui_print "****************************"
exit 0
