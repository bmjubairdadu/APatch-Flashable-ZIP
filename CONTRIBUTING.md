# Contributing to APatch Flashable ZIP

Thanks for your interest! This project repackages the official
[APatch](https://github.com/LyraVoid/APatch) manager (by LyraVoid,
based on [KernelPatch](https://github.com/bmax121/KernelPatch (also bmax121))
...
- **No binaries in git.** `APatch.apk`, `dist/`, `*.zip`, `fp_install/`,
...
- **boot-only.** APatch patches the `kernel`, which always lives in
...
```sh
# 1. Place the official APatch APK next to the repo (same version as APK_URL in tools/build.py)
...
- [ ] Manager app (`me.bmax.apatch`) shows Installed after reboot.
...
`APatch-flash-report.txt` from sdcard if present. Never attach
